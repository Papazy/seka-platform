...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsedData = await new Promise<DosenCsvRow[]>((resolve, reject) => {
      const data: DosenCsvRow[] = [];
      stream
        .pipe(parser)
    ...
    for (const [index, row] of parsedData.entries()) {
      ...
      if (!programStudi) {
        errors.push(
          `Baris ${rowNum - 1}: Program Studi dengan nama ${row.programStudi} tidak ditemukan`,
        );
        continue;
      }
      ...
      const existingDosen = await prisma.dosen.findFirst({
        where: {
          OR: [{ nip: row.nip }, { email: row.email }],
        },
      });

      if (existingDosen) {
        errors.push(
          `Baris ${rowNum}: nip ${row.nip} atau email ${row.email} sudah digunakan`,
        );
        continue;
      }
