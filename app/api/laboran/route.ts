...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingLaboran) {
      return NextResponse.json(
        { error: "Laboran dengan email ini sudah ada" },
        { status: 400 },
      );
    }

    const hashedPassword = await bcyrpt.hash(
      password ?? LABORAN_PASSWORD_DEFAULT,
      10,
    );
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id || !nama || !email) {
      return NextResponse.json(
        { error: "Id, nama dan email harus diisi" },
        { status: 400 },
      );
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { message: "Laboran berhasil dihapus" },
      { status: 200 },
    );
  } catch (error) {
    return NextResponse.json(
