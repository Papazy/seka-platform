...
export async function POST(req: NextRequest){
  try{
    ...
    for (const [index, row] of (parsedData as any[]).entries()){
      ...
      switch (type) {
        case 'dosen':
          ...
          for (const field of requiredFieldsDosen){
            if(!row[field]){
              errors.push(`Baris ${rowNum - 1}: Kolom ${field} tidak boleh kosong`)
              continue
            }
          }
          break;
      ...
      if(!praktikum){
        errors.push(`Baris ${rowNum - 1}: Praktikum dengan nama ${row.praktikum} dan kelas ${row.kelas} tidak ditemukan`)
        continue
      }
      ...
      switch(type) {
        case 'dosen':
          ...
          if (existingDosenPraktikum) {
            errors.push(`Baris ${rowNum - 1}: Dosen sudah terdaftar di praktikum ini`)
            continue
          }
          ...
          try{
            ...
            errors.push(`Baris ${rowNum - 1}: Gagal memasukkan dosen ke dalam praktikum - ${error}`);
            continue
          } 
          break;
        case 'peserta':
          ...
          if(!existingPeserta){
            errors.push(`Baris ${rowNum - 1}: Mahasiswa dengan NPM ${row.npm} tidak ditemukan`)
            continue
          }

          const existingPesertaPraktikum = await prisma.pesertaPraktikum.findFirst({
            where: {
              idPraktikum: praktikum.id,
          ...
          if (existingPesertaPraktikum) {
            errors.push(`Baris ${rowNum - 1}: Peserta sudah terdaftar di praktikum ini`)
            continue
          }
          ...
          try{
            ...
            errors.push(`Baris ${rowNum - 1}: Gagal memasukkan peserta ke dalam praktikum - ${error}`);
            continue
          }
        ...
        case 'asisten':
          ...
          const existingAsistenPraktikum = await prisma.asistenPraktikum.findFirst({
            where: {
              idPraktikum: praktikum.id,
          ...
          if (existingAsistenPraktikum) {
            errors.push(`Baris ${rowNum - 1}: asisten sudah terdaftar di praktikum ini`)
            continue
          }
          ...
          try{
            ...
            errors.push(`Baris ${rowNum - 1}: Gagal memasukkan asisten ke dalam praktikum - ${error}`);
            continue
          }
      }
