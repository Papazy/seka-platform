...
export async function GET(req: NextRequest, { params }: {params: Promise<{ soalId: string }>}) {
  try {
    ...
    if (!soal) return NextResponse.json({ error: 'Soal not found' }, { status: 404 })
    ...
    if (!asisten) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
