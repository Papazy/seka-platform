...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const [index, row] of parsedData.entries()) {
      ...
      const requiredFields: (keyof PraktikumCsvRow)[] = [
        "nama",
        "kodePraktikum",
        "kodeMk",
        "kelas",
      ...
      if (missingFields.length > 0) {
        errors.push(
          `Baris ${rowNum}: Field wajib tidak diisi: ${missingFields.join(", ")}`,
        );
        continue;
      }
      ...
      if (!kodeRegex.test(row.kodePraktikum)) {
        errors.push(
          `Baris ${rowNum}: Format kode praktikum tidak valid (harus 3-10 karakter huruf besar dan angka)`,
        );
        continue;
      }
      ...
      if (!kodeRegex.test(row.kodeMk)) {
        errors.push(
          `Baris ${rowNum}: Format kode mata kuliah tidak valid (harus 3-10 karakter huruf besar dan angka)`,
        );
        continue;
      }
      ...
      if (!validHari.includes(row.jadwalHari.toLowerCase())) {
        errors.push(
          `Baris ${rowNum}: Hari tidak valid (harus: ${validHari.join(", ")})`,
        );
        continue;
      }
      ...
      if (!timeRegex.test(row.jadwalJamMasuk)) {
        errors.push(
          `Baris ${rowNum}: Format jam masuk tidak valid (harus HH:MM)`,
        );
        continue;
      }
      ...
      if (!timeRegex.test(row.jadwalJamSelesai)) {
        errors.push(
          `Baris ${rowNum}: Format jam selesai tidak valid (harus HH:MM)`,
        );
        continue;
      }
      ...
      if (jamSelesai <= jamMasuk) {
        errors.push(
          `Baris ${rowNum}: Jam selesai harus lebih besar dari jam masuk`,
        );
        continue;
      }
      ...
      if (existingPraktikum) {
        errors.push(
          `Baris ${rowNum - 1}: Kode praktikum ${row.kodePraktikum} sudah digunakan`,
        );
        continue;
      }
