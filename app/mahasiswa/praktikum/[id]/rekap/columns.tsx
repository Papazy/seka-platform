...
export const createRekapColumns = (tugasList: Array<{id: number, judul: string}>): ColumnDef<PesertaNilai>[] => [
  {
    header: "No",
    id: "no",
  ...
  ...tugasList.map((tugas) => ({
    ...
    header: tugas.judul,
    cell: ({ row }: { row: any }) => {
      const nilaiTugas = row.original.nilaiPerTugas.find((nt: any) => nt.tugasId === tugas.id)
...
export const createSingleTugasColumns = (
  tugas: {id: number, judul: string, totalSoal: number, totalBobot: number},
  soalList: Array<{id: number, judul: string, bobotNilai: number}>
  ...
  ...soalList.map((soal) => ({
    ...
    cell: ({ row }: { row: any }) => {
      const nilaiTugas = row.original.nilaiPerTugas.find((nt: any) => nt.tugasId === tugas.id)
      const soalScore = nilaiTugas?.soalScores?.find((ss: any) => ss.soalId === soal.id)
  ...
  })),
  {
    ...
    header: 'Nilai',
    cell: ({ row }) => {
      const nilaiTugas = row.original.nilaiPerTugas.find(nt => nt.tugasId === tugas.id)
