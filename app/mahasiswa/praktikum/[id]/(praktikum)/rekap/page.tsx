    ...
    if (selectedView === "kelas") {
      ...
      const csvRows = rekapData.peserta.map((peserta, index) => {
        const tugasNilai = rekapData.tugas.map(tugas => {
          const nilai = peserta.nilaiPerTugas.find(
      ...
      const csvRows = rekapData.peserta.map((peserta, index) => {
        const nilaiTugas = peserta.nilaiPerTugas.find(
          nt => nt.tugasId === selectedView,
        );
        const soalNilai = selectedTugas.soal.map(soal => {
          const soalScore = nilaiTugas?.soalScores?.find(
            ss => ss.soalId === soal.id,
          );
          return soalScore ? `${soalScore.score}` : "0";
        });
      ...
      <div className="bg-white p-6 rounded-lg border">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">
              {rekapData.praktikum.nama} - {rekapData.praktikum.kodePraktikum} -
              Kelas {rekapData.praktikum.kelas}
            </p>
          </div>
        ...
        <div className="mb-6">
          <select
            ...
            onChange={e =>
              setSelectedView(
                e.target.value === "kelas" ? "kelas" : e.target.value,
              )
            }
