...
import { usePesertaPraktikum } from '@/app/hooks/usePesertaPraktikum';
...
export default function PesertaPage() {
  ...
  const [activeTab, setActiveTab] = useState<'pengampu' | 'asisten' | 'peserta'>('peserta');
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="text-lg font-semibold">{pesertaData.peserta.length}</div>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm text-gray-600">Total Asisten</div>
              <div className="text-lg font-semibold">{pesertaData.asisten.length}</div>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm text-gray-600">Total Anggota</div>
              <div className="text-lg font-semibold">{pesertaData.peserta.length + pesertaData.asisten.length}</div>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm text-gray-600">Dosen Pengampu</div>
              <div className="text-lg font-semibold">{pesertaData.dosen.length}</div>
            </div>
          </div>
          ...
          <div className="border-b border-gray-200 mb-6">
            <nav className="-mb-px flex space-x-8">
              {[
                { id: 'peserta', label: `Peserta (${pesertaData.peserta.length})` },
                { id: 'asisten', label: `Asisten (${pesertaData.asisten.length})` },
                { id: 'pengampu', label: `Pengampu (${pesertaData.dosen.length})` },
              ].map((tab) => (
                <button
                  key={tab.id}
