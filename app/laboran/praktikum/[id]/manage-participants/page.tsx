...
export default function ManageParticipantsPage() {
  ...
  const fetchParticipants = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/praktikum/${id}/participants?type=${type}`,
      );
  ...
  const handleAddParticipants = async ({ userIds }: { userIds: number[] }) => {
    try {
      ...
      const newParticipantsResponse = await fetch(
        `/api/praktikum/${id}/participants?type=${type}`,
      );
      ...
      toast.success(
        `Berhasil menambahkan ${result._count} data ${getTabLabel(activeTab)}`,
      );
    } catch (error) {
      toast.error(
        `Terjadi kesalahan saat menambahkan ${getTabLabel(activeTab)}`,
      );
    }
  };
  ...
  const handleDeleteParticipants = async () => {
    try {
      ...
      toast.success(
        `Berhasil menghapus ${result._count} data ${getTabLabel(activeTab)}`,
      );
  ...
  const columns =
    activeTab === "dosen"
      ? createDosenColumns({ selectedIds, handleSelectIds, onClickDelete })
      : createMahasiswaColumns({ selectedIds, handleSelectIds, onClickDelete });
        ...
        <div className="flex items-center ">
          ...
          <div className="">
            ...
            <div className="mt-1 text-sm text-gray-600">
              {praktikum.nama} - Kelas {praktikum.kelas}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="font-bold text-2xl">
                {participants["pesertaPraktikum"].length ||
        ...
        <div className="bg-white border border-gray-200 shadow-sm p-6 rounded-lg">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="font-bold text-2xl">
                {participants["asistenPraktikum"].length ||
        ...
        <div className="bg-white border border-gray-200 shadow-sm p-6 rounded-lg">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="font-bold text-2xl">
                {participants["dosenPraktikum"].length ||
          ...
          {tabList.map(tab => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  handleSwitchTab(tab.key as "peserta" | "asisten" | "dosen")
                }
              >
