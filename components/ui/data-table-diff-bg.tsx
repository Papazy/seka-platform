...
export function DataTableDiffBackground<TData, TValue>({
    columns,
    data,
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {table.getRowModel().rows?.length ? (
                                table.getRowModel().rows.map(row => {
                                    ...
                                    const defaultClassName = row.index % 2 === 0
                                    ...
                                    <tr
                                        key={row.id}
                                        className={`transition-colors ${customClassName || defaultClassName} ${onRowClick ? "cursor-pointer" : ""}`}
                                        onClick={() => onRowClick?.(row.original)}
                                    >
                                        {row.getVisibleCells().map(cell => (
                                            <td key={cell.id} className="px-6 py-4 whitespace-nowrap text-xs">
                                                {flexRender(
