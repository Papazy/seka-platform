...
const sidebarItems: SidebarItem[] = [
  ...
  { name: 'Laboran', href: '/admin/laboran', icon: UsersIcon, shortName: 'Lab' },
  { name: 'Fakultas', href: '/admin/fakultas', icon: BuildingOfficeIcon, shortName: 'Fak' },
  { name: 'Program Studi', href: '/admin/program-studi', icon: AcademicCapIcon, shortName: 'Prodi' },
]
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-white shadow-xl transform transition-transform">
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">SEKA Admin</h2>
                <p className="text-sm text-gray-500">Sistem Evaluasi Kode Aman</p>
              </div>
              <button
                onClick={() => setIsMobileOpen(false)}
      ...
      <div className={`hidden lg:block lg:flex-shrink-0 transition-all duration-300 ${
        ...
        <div className="flex flex-col h-full bg-white border-r border-gray-200 shadow-sm">
          ...
          <div className="p-4 border-t">
            {!isDesktopCollapsed && (
              <button onClick={()=> router.push('/profile')} className="flex items-center space-x-3 mb-3 cursor-pointer">
                <div className="w-8 h-8 bg-[#3ECF8E] rounded-full flex items-center justify-center">
                  <span className="text-white text-sm font-medium">
                    {user?.nama?.charAt(0).toUpperCase()}
                  </span>
