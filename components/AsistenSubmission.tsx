...
import {
  useAsistenSoalSubmissions,
  useUpdateSubmissionScore,
} from "@/hooks/useAsistenSoalSubmissions";

export default function AsistenSubmissions({
  praktikumId,
  tugasId,
  soalId,
}: {
  ...
  const { data: submissions = [], isLoading } = useAsistenSoalSubmissions(
    praktikumId,
    tugasId,
    soalId,
  );
      ...
      {selectedSubmissionId && submissionDetail && (
        <div className="mb-8">
          ...
          <div className="bg-gray-100 rounded-lg p-4 mb-4">
            ...
            <div className="mb-2 flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    editScore !== null &&
                    editScore !== submissionDetail.score
                  ) {
                    updateScore.mutate({
            ...
            <div className="mb-2">
              <span className="font-semibold">Waktu Submit:</span>{" "}
              {new Date(submissionDetail.submittedAt).toLocaleString("id-ID")}
            </div>
            <div className="mb-2">
              ...
              <tbody>
                {submissionDetail.testCaseResult.map((tc: any, idx: number) => (
                  <tr
                    key={tc.id}
                    className={
                      tc.status === "ACCEPTED" ? "bg-green-50" : "bg-red-50"
                    }
                  >
...
function getBestSubmissions(submissions: any[]) {
  ...
  submissions.forEach(sub => {
    ...
    if (
      ...
      (sub.score === map.get(key).score &&
        sub.submittedAt < map.get(key).submittedAt)
    ) {
      map.set(key, sub);
    }
  });
