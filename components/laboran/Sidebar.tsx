// components/laboran/LaboranSidebar.tsx
'use client'
...
const sidebarItems: SidebarItem[] = [
  ...
  { name: 'Praktikum', href: '/laboran/praktikum', icon: BookOpenIcon, shortName: 'Praktikum' },
  { name: 'Mahasiswa', href: '/laboran/mahasiswa', icon: UsersIcon, shortName: 'Mahasiswa' },
  { name: 'Dosen', href: '/laboran/dosen', icon: AcademicCapIcon, shortName: 'Dosen' },
  // { name: 'Tugas', href: '/laboran/tugas', icon: ClipboardDocumentListIcon, shortName: 'Tugas' },
  // { name: 'Peserta', href: '/laboran/peserta', icon: UserGroupIcon, shortName: 'Peserta' },
]
    ...
    <>
      ...
      <div className="lg:hidden flex items-center justify-between p-4 bg-white border-b border-gray-200">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-600">Hi, {user?.nama?.split(' ')[0]}</span>
          <button
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-white shadow-xl">
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">SEKA Laboran</h2>
      ...
      <div className={`hidden lg:block lg:flex-shrink-0 transition-all duration-300 ${
        ...
        <div className="flex flex-col h-full bg-white border-r border-gray-200 shadow-sm">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b">
            <div className={`transition-all duration-300 ${isDesktopCollapsed ? 'opacity-0 w-0' : 'opacity-100'}`}>
              <h2 className="text-lg font-semibold text-gray-900">SEKA Laboran</h2>
              <p className="text-sm text-gray-500">Kelola Praktikum</p>
            </div>
            <button
          ...
          <nav className="flex-1 mt-8 px-4">
            {sidebarItems.map((item) => (
              ...
              >
                <item.icon className={`h-5 w-5 ${isDesktopCollapsed ? 'mx-auto' : 'mr-3'}`} />
                <span className={`transition-all duration-300 ${
                  isDesktopCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'
                }`}>
                  {item.name}
                </span>
          ...
          <div className="p-4 border-t">
            ...
            >
              <ArrowRightOnRectangleIcon className={`h-5 w-5 ${isDesktopCollapsed ? '' : 'mr-3'}`} />
