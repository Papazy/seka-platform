...
export function ImportParticipantsModal({
  ...
  const downloadTemplate = async () => {
    try {
      const response = await fetch(`/api/praktikum/${praktikumId}/participants/template?type=${type}`, {
        credentials: 'include'
      })
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/praktikum/${praktikumId}/participants/import`, {
        method: 'POST',
        credentials: 'include',
        body: formData
      })
  ...
  const getTitle = () => {
    switch (type) {
      case 'peserta': return 'Import Peserta'
      case 'asisten': return 'Import Asisten'  
      case 'dosen': return 'Import Dosen'
      default: return 'Import Peserta'
    }
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-gray-900 mb-2">Format CSV:</h3>
            <div className="space-y-1">
              {getSampleFormat().map((field, index) => (
                <div key={index} className="text-sm text-gray-600">
                  <code className="bg-white px-2 py-1 rounded">{field.label}</code>
            ...
            <p className="text-xs text-gray-500 mt-2">
              * Sistem akan otomatis mencari {type === 'dosen' ? 'dosen' : 'mahasiswa'} berdasarkan {type === 'dosen' ? 'NIP' : 'NPM'}
            </p>
          </div>
