...
const RolePraktikumContext = createContext<RolePraktikumContextType | undefined>(undefined)

export const RolePraktikumProvider = ({children} : {children : ReactNode}) => {
  const {user} = useAuth()
  const {data: mahasiswaRoles = [], isLoading: isLoadingRoles, refetch: refetchRoles} = useFetchMahasiswaRoles(!!user)


  // helper functions
  const checkRole = (praktikumId: string): 'ASISTEN' | 'PESERTA' | null => {
    const mahasiswaRole = mahasiswaRoles.find(role => role.praktikumId === praktikumId)
